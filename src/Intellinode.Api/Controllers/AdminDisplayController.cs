...
[ApiController]
[Route("api/v1/admin/device-config/display")]
[Authorize(Roles = "Admin")]
public sealed class AdminDisplayController : ControllerBase
{
    private readonly IDisplaySettingsService _displaySettingsService;
    private readonly IExceptionLogWriter _exceptionLogWriter;
    private readonly ILogger<AdminDisplayController> _logger;
    private readonly IValidator<DisplayExecuteNowRequest> _executeNowValidator;
    private readonly IValidator<DisplayQueueRequest> _queueValidator;
    private readonly IValidator<DisplayHistoryQuery> _historyQueryValidator;
    ...
    public AdminDisplayController(
        ...
        ILogger<AdminDisplayController> logger,
        IValidator<DisplayExecuteNowRequest> executeNowValidator,
        IValidator<DisplayQueueRequest> queueValidator,
        IValidator<DisplayHistoryQuery> historyQueryValidator,
    ...
    [HttpGet("{macAddress}")]
    public async Task<ActionResult<DisplayCurrentResponse>> GetCurrent(
        string macAddress,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_options.Enabled)
            {
                return NotFound(BuildError("FeatureDisabled", "Display settings endpoint is disabled.", correlationId));
            }
            ...
            var result = await _displaySettingsService.GetCurrentAsync(macAddress.Trim(), cancellationToken);
            if (result.IsSuccess)
            {
    ...
    public async Task<ActionResult<DisplayHistoryResponse>> GetApplyHistory(
        string macAddress,
        [FromQuery] DisplayHistoryQuery query,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _displaySettingsService.GetApplyHistoryAsync(macAddress.Trim(), query, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    [HttpPost("queue")]
    public async Task<ActionResult<DisplayQueueResponse>> Queue(
        [FromBody] DisplayQueueRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return NotFound(BuildError("FeatureDisabled", "Display settings endpoint is disabled.", request.Options.CorrelationId));
            }
            ...
            var result = await _displaySettingsService.QueueAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<DisplayExecuteNowResponse>> ExecuteNow(
        [FromBody] DisplayExecuteNowRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _displaySettingsService.ExecuteNowAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    private static DisplayErrorResponse BuildError(string error, string message, Guid? correlationId) =>
        new()
        {
            Error = error,
            Message = message,
