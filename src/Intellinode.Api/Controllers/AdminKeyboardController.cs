...
[ApiController]
[Route("api/v1/admin/device-config/keyboard")]
[Authorize(Roles = "Admin")]
public sealed class AdminKeyboardController : ControllerBase
{
    private readonly IKeyboardSettingsService _keyboardSettingsService;
    private readonly IExceptionLogWriter _exceptionLogWriter;
    private readonly ILogger<AdminKeyboardController> _logger;
    private readonly IValidator<KeyboardExecuteNowRequest> _executeNowValidator;
    private readonly IValidator<KeyboardQueueRequest> _queueValidator;
    private readonly IValidator<KeyboardHistoryQuery> _historyQueryValidator;
    private readonly KeyboardOptions _options;

    public AdminKeyboardController(
        IKeyboardSettingsService keyboardSettingsService,
        IExceptionLogWriter exceptionLogWriter,
        ILogger<AdminKeyboardController> logger,
        IValidator<KeyboardExecuteNowRequest> executeNowValidator,
        IValidator<KeyboardQueueRequest> queueValidator,
        IValidator<KeyboardHistoryQuery> historyQueryValidator,
        IOptions<KeyboardOptions> options)
    {
        _keyboardSettingsService = keyboardSettingsService;
    ...
    [HttpGet("{macAddress}")]
    public async Task<ActionResult<KeyboardCurrentResponse>> GetCurrent(
        string macAddress,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_options.Enabled)
            {
                return NotFound(BuildError("FeatureDisabled", "Keyboard settings endpoint is disabled.", correlationId));
            }
            ...
            var result = await _keyboardSettingsService.GetCurrentAsync(macAddress.Trim(), cancellationToken);
            if (result.IsSuccess)
            {
    ...
    public async Task<ActionResult<KeyboardHistoryResponse>> GetApplyHistory(
        string macAddress,
        [FromQuery] KeyboardHistoryQuery query,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _keyboardSettingsService.GetApplyHistoryAsync(macAddress.Trim(), query, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    [HttpPost("queue")]
    public async Task<ActionResult<KeyboardQueueResponse>> Queue(
        [FromBody] KeyboardQueueRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return NotFound(BuildError("FeatureDisabled", "Keyboard settings endpoint is disabled.", request.Options.CorrelationId));
            }
            ...
            var result = await _keyboardSettingsService.QueueAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<KeyboardExecuteNowResponse>> ExecuteNow(
        [FromBody] KeyboardExecuteNowRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _keyboardSettingsService.ExecuteNowAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    private static KeyboardErrorResponse BuildError(string error, string message, Guid? correlationId) =>
        new()
        {
            Error = error,
            Message = message,
