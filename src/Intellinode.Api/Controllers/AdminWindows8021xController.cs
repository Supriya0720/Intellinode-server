...
[ApiController]
[Route("api/v1/admin/device-config/windows-802-1x")]
[Authorize(Roles = "Admin")]
public sealed class AdminWindows8021xController : ControllerBase
{
    private readonly IWindows8021xSettingsService _windows8021xSettingsService;
    private readonly IExceptionLogWriter _exceptionLogWriter;
    private readonly ILogger<AdminWindows8021xController> _logger;
    private readonly IValidator<Windows8021xExecuteNowRequest> _executeNowValidator;
    private readonly IValidator<Windows8021xQueueRequest> _queueValidator;
    private readonly IValidator<Windows8021xHistoryQuery> _historyQueryValidator;
    private readonly Windows8021xOptions _options;

    public AdminWindows8021xController(
        IWindows8021xSettingsService windows8021xSettingsService,
        IExceptionLogWriter exceptionLogWriter,
        ILogger<AdminWindows8021xController> logger,
        IValidator<Windows8021xExecuteNowRequest> executeNowValidator,
        IValidator<Windows8021xQueueRequest> queueValidator,
        IValidator<Windows8021xHistoryQuery> historyQueryValidator,
        IOptions<Windows8021xOptions> options)
    {
        _windows8021xSettingsService = windows8021xSettingsService;
    ...
    [HttpGet("{macAddress}")]
    public async Task<ActionResult<Windows8021xCurrentResponse>> GetCurrent(
        string macAddress,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_options.Enabled)
            {
                return NotFound(BuildError("FeatureDisabled", "Windows 802.1X settings endpoint is disabled.", correlationId));
            }
            ...
            var result = await _windows8021xSettingsService.GetCurrentAsync(macAddress.Trim(), cancellationToken);
            if (result.IsSuccess)
            {
    ...
    public async Task<ActionResult<Windows8021xHistoryResponse>> GetApplyHistory(
        string macAddress,
        [FromQuery] Windows8021xHistoryQuery query,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _windows8021xSettingsService.GetApplyHistoryAsync(macAddress.Trim(), query, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    [HttpPost("queue")]
    public async Task<ActionResult<Windows8021xQueueResponse>> Queue(
        [FromBody] Windows8021xQueueRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return NotFound(BuildError("FeatureDisabled", "Windows 802.1X settings endpoint is disabled.", request.Options.CorrelationId));
            }
            ...
            var result = await _windows8021xSettingsService.QueueAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<Windows8021xExecuteNowResponse>> ExecuteNow(
        [FromBody] Windows8021xExecuteNowRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _windows8021xSettingsService.ExecuteNowAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    private static Windows8021xErrorResponse BuildError(string error, string message, Guid? correlationId) =>
        new()
        {
            Error = error,
            Message = message,
