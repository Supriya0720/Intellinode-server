...
[ApiController]
[Route("api/v1/admin/device-config/windows-computer-name")]
[Authorize(Roles = "Admin")]
public sealed class AdminWindowsComputerNameController : ControllerBase
{
    private readonly IWindowsComputerNameSettingsService _computerNameSettingsService;
    private readonly IExceptionLogWriter _exceptionLogWriter;
    private readonly ILogger<AdminWindowsComputerNameController> _logger;
    private readonly IValidator<WindowsComputerNameExecuteNowRequest> _executeNowValidator;
    private readonly IValidator<WindowsComputerNameExecuteNowBulkRequest> _executeNowBulkValidator;
    private readonly IValidator<WindowsComputerNameExecuteNowGroupRequest> _executeNowGroupValidator;
    private readonly IValidator<WindowsComputerNameQueueRequest> _queueValidator;
    private readonly IValidator<WindowsComputerNameHistoryQuery> _historyQueryValidator;
    private readonly WindowsComputerNameOptions _options;

    public AdminWindowsComputerNameController(
        IWindowsComputerNameSettingsService computerNameSettingsService,
        IExceptionLogWriter exceptionLogWriter,
        ILogger<AdminWindowsComputerNameController> logger,
        IValidator<WindowsComputerNameExecuteNowRequest> executeNowValidator,
        IValidator<WindowsComputerNameExecuteNowBulkRequest> executeNowBulkValidator,
        IValidator<WindowsComputerNameExecuteNowGroupRequest> executeNowGroupValidator,
        IValidator<WindowsComputerNameQueueRequest> queueValidator,
        IValidator<WindowsComputerNameHistoryQuery> historyQueryValidator,
        IOptions<WindowsComputerNameOptions> options)
    {
        _computerNameSettingsService = computerNameSettingsService;
    ...
    [HttpGet("{macAddress}")]
    public async Task<ActionResult<WindowsComputerNameCurrentResponse>> GetCurrent(
        string macAddress,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_options.Enabled)
            {
                return NotFound(BuildError("FeatureDisabled", "Windows computer name endpoint is disabled.", correlationId));
            }
            ...
            var result = await _computerNameSettingsService.GetCurrentAsync(macAddress.Trim(), cancellationToken);
            if (result.IsSuccess)
            {
    ...
    public async Task<ActionResult<WindowsComputerNameHistoryResponse>> GetApplyHistory(
        string macAddress,
        [FromQuery] WindowsComputerNameHistoryQuery query,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _computerNameSettingsService.GetApplyHistoryAsync(macAddress.Trim(), query, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    [HttpPost("queue")]
    public async Task<ActionResult<WindowsComputerNameQueueResponse>> Queue(
        [FromBody] WindowsComputerNameQueueRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return NotFound(BuildError("FeatureDisabled", "Windows computer name endpoint is disabled.", request.Options.CorrelationId));
            }
            ...
            var result = await _computerNameSettingsService.QueueAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsComputerNameExecuteNowResponse>> ExecuteNow(
        [FromBody] WindowsComputerNameExecuteNowRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _computerNameSettingsService.ExecuteNowAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsComputerNameBulkResponse>> ExecuteNowBulk(
        [FromBody] WindowsComputerNameExecuteNowBulkRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _computerNameSettingsService.ExecuteNowBulkAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsComputerNameBulkResponse>> ExecuteNowGroup(
        Guid groupId,
        [FromBody] WindowsComputerNameExecuteNowGroupRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _computerNameSettingsService.ExecuteNowGroupAsync(groupId, request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    private static WindowsComputerNameErrorResponse BuildError(string error, string message, Guid? correlationId) =>
        new()
        {
            Error = error,
            Message = message,
