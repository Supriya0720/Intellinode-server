...
[ApiController]
[Route("api/v1/admin/device-config/windows-ethernet-setup")]
[Authorize(Roles = "Admin")]
public sealed class AdminWindowsEthernetSetupController : ControllerBase
{
    private readonly IWindowsEthernetSetupSettingsService _ethernetSetupSettingsService;
    private readonly IExceptionLogWriter _exceptionLogWriter;
    private readonly ILogger<AdminWindowsEthernetSetupController> _logger;
    private readonly IValidator<WindowsEthernetSetupExecuteNowRequest> _executeNowValidator;
    private readonly IValidator<WindowsEthernetSetupQueueRequest> _queueValidator;
    private readonly IValidator<WindowsEthernetSetupHistoryQuery> _historyQueryValidator;
    private readonly IValidator<WindowsEthernetSetupExecuteNowBulkRequest> _executeNowBulkValidator;
    private readonly IValidator<WindowsEthernetSetupExecuteNowGroupRequest> _executeNowGroupValidator;
    private readonly WindowsEthernetSetupOptions _options;

    public AdminWindowsEthernetSetupController(
        IWindowsEthernetSetupSettingsService ethernetSetupSettingsService,
        IExceptionLogWriter exceptionLogWriter,
        ILogger<AdminWindowsEthernetSetupController> logger,
        IValidator<WindowsEthernetSetupExecuteNowRequest> executeNowValidator,
        IValidator<WindowsEthernetSetupQueueRequest> queueValidator,
        IValidator<WindowsEthernetSetupHistoryQuery> historyQueryValidator,
        IValidator<WindowsEthernetSetupExecuteNowBulkRequest> executeNowBulkValidator,
        IValidator<WindowsEthernetSetupExecuteNowGroupRequest> executeNowGroupValidator,
        IOptions<WindowsEthernetSetupOptions> options)
    {
        _ethernetSetupSettingsService = ethernetSetupSettingsService;
    ...
    [HttpGet("{macAddress}")]
    public async Task<ActionResult<WindowsEthernetSetupCurrentResponse>> GetCurrent(
        string macAddress,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_options.Enabled)
            {
                return NotFound(BuildError("FeatureDisabled", "Windows ethernet setup endpoint is disabled.", correlationId));
            }
            ...
            var result = await _ethernetSetupSettingsService.GetCurrentAsync(macAddress.Trim(), cancellationToken);
            if (result.IsSuccess)
            {
    ...
    public async Task<ActionResult<WindowsEthernetSetupHistoryResponse>> GetApplyHistory(
        string macAddress,
        [FromQuery] WindowsEthernetSetupHistoryQuery query,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _ethernetSetupSettingsService.GetApplyHistoryAsync(macAddress.Trim(), query, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    [HttpPost("queue")]
    public async Task<ActionResult<WindowsEthernetSetupQueueResponse>> Queue(
        [FromBody] WindowsEthernetSetupQueueRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return NotFound(BuildError("FeatureDisabled", "Windows ethernet setup endpoint is disabled.", request.Options.CorrelationId));
            }
            ...
            var result = await _ethernetSetupSettingsService.QueueAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsEthernetSetupExecuteNowResponse>> ExecuteNow(
        [FromBody] WindowsEthernetSetupExecuteNowRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _ethernetSetupSettingsService.ExecuteNowAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsEthernetSetupBulkResponse>> ExecuteNowBulk(
        [FromBody] WindowsEthernetSetupExecuteNowBulkRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _ethernetSetupSettingsService.ExecuteNowBulkAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsEthernetSetupBulkResponse>> ExecuteNowGroup(
        Guid groupId,
        [FromBody] WindowsEthernetSetupExecuteNowGroupRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _ethernetSetupSettingsService.ExecuteNowGroupAsync(groupId, request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    private static WindowsEthernetSetupErrorResponse BuildError(string error, string message, Guid? correlationId) =>
        new()
        {
            Error = error,
            Message = message,
