...
[ApiController]
[Route("api/v1/admin/device-config/windows-region-location")]
[Authorize(Roles = "Admin")]
public sealed class AdminWindowsRegionLocationController : ControllerBase
{
    private readonly IWindowsRegionLocationSettingsService _regionLocationSettingsService;
    private readonly IExceptionLogWriter _exceptionLogWriter;
    private readonly ILogger<AdminWindowsRegionLocationController> _logger;
    private readonly IValidator<WindowsRegionLocationExecuteNowRequest> _executeNowValidator;
    private readonly IValidator<WindowsRegionLocationExecuteNowBulkRequest> _executeNowBulkValidator;
    private readonly IValidator<WindowsRegionLocationExecuteNowGroupRequest> _executeNowGroupValidator;
    private readonly IValidator<WindowsRegionLocationQueueRequest> _queueValidator;
    private readonly IValidator<WindowsRegionLocationHistoryQuery> _historyQueryValidator;
    private readonly WindowsRegionLocationOptions _options;

    public AdminWindowsRegionLocationController(
        IWindowsRegionLocationSettingsService regionLocationSettingsService,
        IExceptionLogWriter exceptionLogWriter,
        ILogger<AdminWindowsRegionLocationController> logger,
        IValidator<WindowsRegionLocationExecuteNowRequest> executeNowValidator,
        IValidator<WindowsRegionLocationExecuteNowBulkRequest> executeNowBulkValidator,
        IValidator<WindowsRegionLocationExecuteNowGroupRequest> executeNowGroupValidator,
        IValidator<WindowsRegionLocationQueueRequest> queueValidator,
        IValidator<WindowsRegionLocationHistoryQuery> historyQueryValidator,
        IOptions<WindowsRegionLocationOptions> options)
    {
        _regionLocationSettingsService = regionLocationSettingsService;
    ...
    [HttpGet("{macAddress}")]
    public async Task<ActionResult<WindowsRegionLocationCurrentResponse>> GetCurrent(
        string macAddress,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_options.Enabled)
            {
                return NotFound(BuildError("FeatureDisabled", "Windows region and location endpoint is disabled.", correlationId));
            }
            ...
            var result = await _regionLocationSettingsService.GetCurrentAsync(macAddress.Trim(), cancellationToken);
            if (result.IsSuccess)
            {
    ...
    public async Task<ActionResult<WindowsRegionLocationHistoryResponse>> GetApplyHistory(
        string macAddress,
        [FromQuery] WindowsRegionLocationHistoryQuery query,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _regionLocationSettingsService.GetApplyHistoryAsync(macAddress.Trim(), query, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    [HttpPost("queue")]
    public async Task<ActionResult<WindowsRegionLocationQueueResponse>> Queue(
        [FromBody] WindowsRegionLocationQueueRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return NotFound(BuildError("FeatureDisabled", "Windows region and location endpoint is disabled.", request.Options.CorrelationId));
            }
            ...
            var result = await _regionLocationSettingsService.QueueAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsRegionLocationExecuteNowResponse>> ExecuteNow(
        [FromBody] WindowsRegionLocationExecuteNowRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _regionLocationSettingsService.ExecuteNowAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsRegionLocationBulkResponse>> ExecuteNowBulk(
        [FromBody] WindowsRegionLocationExecuteNowBulkRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _regionLocationSettingsService.ExecuteNowBulkAsync(request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    public async Task<ActionResult<WindowsRegionLocationBulkResponse>> ExecuteNowGroup(
        Guid groupId,
        [FromBody] WindowsRegionLocationExecuteNowGroupRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            TryGetAdminId(out var adminId);
            var result = await _regionLocationSettingsService.ExecuteNowGroupAsync(groupId, request, adminId, cancellationToken);
            if (result.IsSuccess)
            {
                return Ok(result.Response);
            }
    ...
    private static WindowsRegionLocationErrorResponse BuildError(string error, string message, Guid? correlationId) =>
        new()
        {
            Error = error,
            Message = message,
