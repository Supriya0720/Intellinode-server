...
public sealed class WindowsUserInterfaceTaskAckHandler
{
    public const int MaxReasonLength = 500;

    private readonly IntellinodeDbContext _dbContext;
    private readonly EffectiveAgentSettingsResolver _resolver;
    private readonly ILogger<WindowsUserInterfaceTaskAckHandler> _logger;

    public WindowsUserInterfaceTaskAckHandler(
        IntellinodeDbContext dbContext,
        EffectiveAgentSettingsResolver resolver,
        ILogger<WindowsUserInterfaceTaskAckHandler> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var settings = await ResolveUserInterfaceSettingsAsync(device, cancellationToken);
        if (settings is null)
        {
            _logger.LogWarning(
                "User interface task {TaskId} ack ignored: no device_windows_user_interface_settings row for device {DeviceId}",
                task.Id,
                device.Id);
            return;
        }
        ...
        switch (terminalStatus)
        {
            ...
            case DeviceTaskStatus.Failed:
                ...
                settings.UpdatedUtc = now;
                var failureMessage = settings.LastApplyMessage ?? "Autologon apply failed.";
                await _resolver.WriteApplyLogAsync(
                    device.Id,
                    SettingsKind.WindowsUserInterface,
                    settings.SettingsVersion,
    ...
    internal static bool IsUserInterfaceTask(DeviceTask task) =>
        string.Equals(
            task.ModuleName,
            WindowsUserInterfaceModuleConstants.ModuleName,
    ...
    private async Task<DeviceWindowsUserInterfaceSettings?> ResolveUserInterfaceSettingsAsync(
        Device device,
        CancellationToken cancellationToken)
    {
        if (device.WindowsUserInterfaceSettings is not null)
        {
            return device.WindowsUserInterfaceSettings;
        }
        ...
        {
            device.WindowsUserInterfaceSettings = settings;
        }
